# The Shogi Opening Analyzer
<p align="center">
  <img width = "574" height = "400" src = "https://github.com/Destaq/opening_analysis/raw/master/images/main_image.png">
 </p>

## About
A program that will produce a graphical sunburst chart of shogi openings from the PSN that is provided to it.

You can upload your own PSN files and then create a multi-level piechart to your inputted depth that will show each move by its relative popularity.
...
- **Run easily** with the PyPI module, `shogi-graph`
...
- Requires Plotly and psnparser
- To run as PyPI module, have this projects PyPI package installed

You can install Plotly and psnparser using PyPi, as long as you are an admin user on your device. To do so, simply type `pip install plotly` and `pip install psnparser` into your terminal.

If you would like to simply run this code from Python IDE, then type `pip install shogi-graph` (make sure you have the other modules installed as well).

### How to Use

#### Using directly from the Github Repo
1. Download the repo (make sure you have fulfilled the prerequisites)
2. Download the PSN game you want to analyze (or use one of the examples). Make sure it is downloaded in the same folder as the repo.
3. Import `chart.py`
4. Run the function `shogi_graph.graph(*path to file*, depth=5, shade = True, fragmentation_percentage=0.0032, should_defragment=False, custom_branching=False, should_download = False, download_format = 'png', download_name = 'fig1')`
- Note that the path to the psn file must be relative to the current directory python is running in. For example, if you have the psn downloaded on your Desktop, you could easily solve this issue by navigating to Desktop within terminal (cd Desktop). From there, you can run *path to file* simply by typing in the filename (assuming it is in Desktop). If this is confusing, just type in the full path.
